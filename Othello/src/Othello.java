...
public class Othello {
	...
	public static int[] getMove(char player, Gameboard theBoard){
		...
		int[] move = {row, col};
		if (moveIsPossible(move, theBoard, player)){
			return move;
		}
		else{ 
	...
	public static boolean moveIsPossible(int[] move, Gameboard theBoard, char player){
		ArrayList<int[]> possibleMoves = theBoard.findPossibleMoves(player);
		for (int[] posMove : possibleMoves){
			if(move[0]==posMove[0] && move[1]==posMove[1]){
				return true;
			}
		}
		return false;
	...
	public static void main(String[] args){
		...
		while (playAgain){
			...
			while (theBoard.findPossibleMoves('X').size()>0 ||
					theBoard.findPossibleMoves('O').size()>0){
				ArrayList<int[]> possibleMoves = theBoard.findPossibleMoves(player);
